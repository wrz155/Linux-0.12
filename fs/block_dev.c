...
// 设备数据块总数指针数组。每个指针项指向指定主设备号的总块数数组hd_sizes[]。该总块
// 数数组每一项对应子设备号确定的一个子设备上所拥有的数据块总数（1块大小 = 1KB）。
extern int *blk_size[];
...
//		buf - 用户空间中缓冲区地址
//		count - 要传送的字节数
// 返回	已写入字节数。若没有写入任何字节或出错，则返回出错号。
// 对于内核来说，写操作是向高速缓冲区中写入数据。什么时候数据最终写入设备是高速缓冲管
// 理程序决定并处理的。另外，因为块设备是以块为单位进行读写，因此对于写开始位置不处于
// 块起始处时，需要先将开始字节所在整个块读出，然后将需要写的数据从开始处填写满该块，
// 再将完整的一块数据写盘（即交由高速缓冲程序去处理）。
int block_write(int dev, long * pos, char * buf, int count)
{
	// 首先由文件中位置pos换算成开始写盘块的块序号block，并求出需写第1字节在该块中的
	// 偏移位置offset。
	...
	// 在写一个块设备文件时，要求写的总数据块数当然不能超过指定设备上容许的最大数据
	// 块总数。因此这里首先取出指定设备的块总数size来比较和限制函数参数给定的读入数
	// 据长度。如果系统中没有对设备指定长度，就使用默认长度0x7fffffff（2GB个块）。
	if (blk_size[MAJOR(dev)]) {
		size = blk_size[MAJOR(dev)][MINOR(dev)];
	} else {
		size = 0x7fffffff;
	}
	// 然后针对要写入的字节数count，循环执行以下操作，直到数据全部写入。在循环执行过
	// 程中，若当前写入数据的块号已经大于或等于指定设备的总块数，则返回已写字节数并
	// 退出。然后再计算在当前处理的数据块中可写入的字节数。如果需要写入的字节数填不
	// 满一块，那么就只需写count字节。如果正好写1块数据内容，则直接申请1块高速缓冲块，
	// 并把用户数据放入即可。否则就需要读入将被写入部分数据的数据块，并预读下两块数
	// 据。然后将块号递增1,为下次操作做好准备。如果缓冲块操作失败，则返回已写字节数，
...
int block_read(int dev, unsigned long * pos, char * buf, int count)
{
	...
	// 在读一个块设备文件时，要求读的总数据块数当然不能超过指定设备上容许的最大数据
	// 块总数。因此这里首先取出指定设备的块总数size来比较和限制函数参数给定的读入数
	// 据长度。如果系统中没有对设备指定长度，就使用默认长度0x7fffffff（2GB个块）。
	if (blk_size[MAJOR(dev)])
		size = blk_size[MAJOR(dev)][MINOR(dev)];
