...
// 第23行定义了一个局部寄存器变量.该变量将被保存在eax寄存器中,以便于高效访问和操作.第24行上指令bt用于对位进行
// 测试(Bit Test).它会把地址addr(%3)和位偏移量bitnr(%2)指定的位的值放入进位标志CF中.指令setb用于根据进位标志
// CF设置操作数%al.如果CF = 1则%al = 1,否则%al = 0.
...
// 以下3个函数(lock_super(),free_super()和wait_on_super())的作用与inode.c文件中头3个函数的作用相同,只是这里操
// 作的对象换成了超级块.

// 锁定超级块.
// 如果超级块已被锁定,则将当前任务置为不可中断的等待状态,并添加到该超级块等待队列s_wait中.直到该超级块解锁并明确
// 地唤醒本任务.然后对其上锁.
...
// 睡眠等待超级块解锁.
// 如果超级块已被锁定,则将当前任务置为不可中断的等待状态,并添加到该超级块的等待队列s_wait中.直到该超级块解锁并明确
// 地唤醒本任务.
...
struct super_block * get_super(int dev)
{
	...
	// 首先判断参数给出设备的有效性.若设备号为0则返回空指针.然后让s指向超级块数组起始处,开始搜索整个超级块数组,以
	// 寻找指定设备dev的超级块.
	if (!dev)
		return NULL;
	s = 0 + super_block;
	while (s < NR_SUPER + super_block)
		// 如果当前搜索项是指定设备的超级块,即该超级块的设备号字段值与函数参数指定的相同,则先等待该超级块解锁(若
		// 已被其他进程上锁的话).在等待期间,该超级块项有可能被其他设备使用,因此等待返回之后需再判断一次是否是指定
		// 设备的超级块,如果是则返回该超级块的指针.否则就重新对超级块数组再搜索一遍,因此此时s需重又指向超级块数组
		// 开始处.
		if (s->s_dev == dev) {
			wait_on_super(s);
			if (s->s_dev == dev)
...
void put_super(int dev)
{
	...
	// 首先判断参数的有效性和合法性。如果指定设备是根文件系统设备，则显示警告信息“根系统盘改变了，准备生死决战吧”，
	// 并返回。然后在超级块表中寻找指定设备号的文件系统超级块。如果找不到指定设备超级块，则返回。另外，如果该超级块
	// 指明该文件系统所安装到的i节点还没有被处理过，则显示警告信息并返回。在文件系统卸载（umount）操作中，s_imount会
	// 先被置成NULL以后才会调用本函数。
	if (dev == ROOT_DEV) {
	...
	// 然后在找到指定设备的超级块之后，我们先锁定该超级块，再置该超级块对应的设备号字段s_dev为0，也即释放该设备上的
	// 文件系统超级块。然后释放该超级块占用的其他内核资源，即释放该设备上文件系统i节点位图和逻辑位图在缓冲区中所占用
	// 的缓冲块。下面常数符号I_MAP_SLOTS和Z_MAP_SLOTS均等于8，用于分别指明i节点位图和逻辑块位图占用的磁盘逻辑块数。
...
// 读取指定设备的超级块.
// 如果指定设备dev上的文件系统超级块已经在超级块表中,则直接返回该超级块项的指针.否则就从设备dev上读取超级块到缓冲块中,
// 并复制到超级块表中.并返回超级块指针.
static struct super_block * read_super(int dev)
{
	...
	// 首先判断参数的有效性.如果没有指明设备,则返回空指针.然后检查该设备是否可更换过盘片(也即是否是软盘设备).如果更换
	// 过盘,则高速缓冲区有关该设备的所有缓冲块均失效,需要进行失效处理,即释放原来加载的文件系统.
	if (!dev)
		return NULL;
	check_disk_change(dev);
	// 如果设备的超级块已经在超级块表中,则直接返回该超级块的指针.否则,首先在超级块数组中找出一个空项(也即字段s_dev=0
	// 的项).如果数组已经占满则返回空指针.
	if (s = get_super(dev))
		return s;
	...
	// 在超级块数组中找到空项之后,就将该超级块项用于指定设备dev上的文件系统.于是对该超级块结构中的内存字段进行部分初
	// 始化处理.
	...
	s->s_rd_only = 0;
	s->s_dirt = 0;
	// 然后锁定该超级块,并从设备上读取超级块信息到bh指向的缓冲块中.超级块位于块设备的第2个逻辑块(１号块)中,(第１个
	// 是引导盘块).如果读超级块操作失败,则释放上面选定的超级块数组中的项(即置s_dev=0),并解锁该项,返回空指针退出.否则
	// 就将设备上读取的超级块信息从缓冲块数据区复制到超级块数组相应项结构中.并释放存放读取信息的高速缓冲块.
	lock_super(s);
	if (!(bh = bread(dev, 1))) {
		s->s_dev = 0;
		free_super(s);
	...
	*((struct d_super_block *) s) =
	...
	brelse(bh);
	// 现在我们从设备dev上得到了文件系统的超级块,于是开始检查这个超级块的有效性并从设备上读取i节点位图和逻辑块位图等
	// 信息.如果所读取的超级块的文件系统魔数字段不对,说明设备上不是正确的文件系统,因此向上面一样,释放上面选定的超级块
	// 数组中的项,并解锁该项,返回空指针退出.
