...
int file_read(struct m_inode * inode, struct file * filp, char * buf, int count)
{
	...
	// 得到包含文件当前读写位置的数据块在设备上对应的逻辑块号nr。若nr不为0,则从i节点指定的设备上读取该逻辑块。
	...
	while (left) {
		...
		// 然后和现在还需读取的字节数left作比较，其中小值即为本次操作需读取的字节数chars。如果
		// （BLOCK_SIZE - nr）> left，则说明该块是需要读取的最后一块数据，反之还需要读取下一块数据。之后调整读写
		// 文件指针。指针前移此次将读取的字节数chars。剩余字节数left相应减去chars。
		nr = filp->f_pos % BLOCK_SIZE;
		chars = MIN( BLOCK_SIZE - nr , left);
		filp->f_pos += chars;
		left -= chars;
		// 若上面从设备上读到了数据，则将p指向缓冲块中开始读取数据的位置，并且复制chars字节到用户缓冲区buf中。否
		// 则往用户缓冲区中填入chars个值字节。
		if (bh) {
			char * p = nr + bh->b_data;
			while (chars-- > 0)
