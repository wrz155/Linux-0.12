 ...
 */
/*
 * OK，Peter编制了复杂但很直观的多个_wait()函数。我对这些函数进行了改写，以使之更简洁：这些代码可能不容易看懂，
 * 但是其中应该不会存在竞争条件问题，并且很实际。如果你能理解这里编制的代码，那么就说明你已经理解Linux中睡眠/
 * 唤醒的工作机制。
 *
 * 两个很简单的过程，add_wait()和free_wait()执行了主要操作。在整个select处理过程中我们不得不禁止中断。但是这样
 * 做并不会带来太多的损失：因为当我们不在执行本任务时睡眠状态会自动释放中断（即其他任务会使用自己EFLAGS中的中
 * 断标志）。
 */
...
// 把未准备好描述符的等待队列指针加入等待表wait_table中。参数*wait_address是与描述符相关的等待队列头指针。例如
// tty读缓冲队列secondary的等待队列头指针是proc_list。参数p是do_select()中定义的等待表结构指针。
static void add_wait(struct task_struct ** wait_address, select_table * p)
{
	int i;

	// 首先判断描述符是否有对应的等待队列，若无则返回。然后在等待表中搜索参数指定的等待队列指针是否已经在等待表
	// 中设置过，若设置过也立刻返回。这个判断主要是针对管道文件描述符。例如若一个管道在等待可以进行读操作，那么
	// 其必定可以立刻进行写操作。
	if (!wait_address)
		return;
	for (i = 0 ; i < p->nr ; i++)
