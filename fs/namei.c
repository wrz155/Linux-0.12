...
// "hello world"[2] = 2["helloworld"] = *("helloworld"+2) = 'l'
// "\004\002\006\377" 代表4个字符(这里只是以8进制显示而已), & O_ACCMODE(3) 用于限制数组访问
// '\004' = r, 可读
// '\002' = w, 可写
// '\006' = rw，可读写
// '\377' = rwxrwxrwx
...
/* #define NO_TRUNCATE */ // 这个宏用于控制（当文件名长度 > NAME_LEN 是否截断）
 ...
 * 不在内核空间).因而我们只能使用match().问题不大,match()同样也处理
 * 一些完整的测试.
 *
...
static int match(int len, const char * name, struct dir_entry * de)
{
	...
	/* 有点皮，长度不同就是不匹配，de->name[len]!='\0',说明de->name更长 */
...
// 		name - 文件名;
//		namelen - 文件名长度;
// 该函数在指定目录的数据(文件)中搜索指定文件名的目录项.并对指定文件名是'..'的情况根据当前进行的相关设置进行特殊处理.
...
#endif
	// 首先计算本目录中目录项项数entries.目录i节点i_size字段中含有本目录包含的数据长度,因此其除以一个目录项的长度
	// (16字节)即可得到该目录中目录项数.然后置空返回目录项结构指针.
	...
	*res_dir = NULL;
	...
	if (namelen == 2 && get_fs_byte(name) == '.' && get_fs_byte(name + 1) == '.') {
		...
		if ((*dir) == current->root) {
			...
			/* 在一个安装点上的'..'将导致目录交换到被安装文件系统的目录i节点上.注意! 由于我们设置了mounted标志,因
			而我们能够放回该新目录 */
	...
	while (i < entries) {
		if ((char *)de >= BLOCK_SIZE + bh->b_data) {
			...
			if (!(block = bmap(*dir, i / DIR_ENTRIES_PER_BLOCK)) ||
				...
				i += DIR_ENTRIES_PER_BLOCK;// 该逻辑块不存在，回到循环开始 bh==NULL???
				continue;
			}
			de = (struct dir_entry *) bh->b_data;
		}
	...
	const char * name, int namelen, struct dir_entry ** res_dir)
{
	...
	*res_dir = NULL;	// 用于返回目录项结构指针。
	...
	if (namelen > NAME_LEN)
		namelen = NAME_LEN;
#endif
	// 现在我们开始操作，向指定目录中添加一个指定文件名的目录项。因此我们需要先读取目录的数据，即取出目录
	// i节点对应块设备数据区中的数据块（逻辑块）信息。这些逻辑块的块号保存在i节点结构的i_zone[9]数组中。
	// 我们先取其第1个块号。如果目录i节点指向的第一个直接磁盘块号为0,则说明该目录竟然不含数据，这不正常。
	// 于是返回NULL退出。否则我们就从节点所在设备读取指定的目录项数据块。如果不成功，则也返回NULL退出。另
	// 外，如果参数提供的文件名长度等于0,则也返回NULL退出。
	if (!namelen)
		return NULL;
	if (!(block = dir->i_zone[0]) || !(bh = bread(dir->i_dev, block)) )
		return NULL;
	// 此时我们就在这个目录i节点数据块中循环查找最后未使用的空目录项。首先让目录项结构指针de指向缓冲块中的
	// 数据块部分，即第一个目录项处。其中i是目录中的目录项索引号，在循环开始时初始化为0。
	i = 0;
	de = (struct dir_entry *) bh->b_data;
	while (1) {
		// 如果当前目录项数据块已经搜索完毕，但还没有找到需要的空目录项，则释放当前目录项数据块，再读入目
		// 录的下一个逻辑块。如果对应的逻辑块不存在就创建一块。若读取或创建操作失败则返回空。如果此次读取
		// 的磁盘逻辑块数据返回的缓冲块指针为空，说明这块逻辑块可能是因为不存在而新创建的空块，则把目录项
		// 索引值加上一块逻辑块所能容纳的目录项数DIR_ENTRIES_PER_BLOCK，用以跳过该块并继续搜索。否则说明
		// 新读入的块上有目录项数据，于是让目录项结构指针de指向该块的缓冲块数据部分，然后在其中继续搜索。
		// 其中i/DIR_ENTRIES_PER_BLOCK可计算得到当前搜索的目录项i所在目录文件中的块号，而create_block()函
		// 数（inode.c）则可读取或创建出在设备上对应的逻辑块。
		if ((char *)de >= BLOCK_SIZE + bh->b_data) {
			brelse(bh);
			bh = NULL;
		...
		// 如果当前所操作的目录项序号i乘上结构大小所得长度值已经超过目录i节点信息所指出的目录数据长度值
		// i_size，则说明整个目录文件数据中没有由于删除文件留下的空目录项，因此我们只能把需要添加的新目
		// 录项附加到目录文件数据的末端处。于是对该处目录项进行设置（置该目录项的i节点指针为空），并更新
		// 该目录文件的长度值（加上一个目录项的长度），然后设置目录的i节点已修改标志，再更新该目录的改变
		// 时间为当前时间。
		...
		// 若当前搜索的目录项de的i节点为空，则表示找到一个还未使用的空闲目录项或是添加的新目录项。于是更新
		// 目录的修改时间为当前时间，并从用户数据区复制文件名到该目录项的文件名字段，置含有本目录项的相应高
		// 速缓冲块已修改标志。返回该目录项的指针以及该高速缓冲块的指针，退出。
		if (!de->inode) {
...
static struct m_inode * follow_link(struct m_inode * dir, struct m_inode * inode)
{
	...
	// 首先判断函数参数的有效性.如果没有给出目录i节点,我们就使用进程任务结构中设置的根i节点,并把链接数增1.如果
	// 没有给出目录项i节点,则放回目录i节点后返回NULL.如果指定目录项不是一个符号链接,就直接返回目录项对应的i节
	// 点inode.
	if (!dir) {
		dir = current->root;
		dir->i_count++;
