...
// 下面函数名前关键字volatile用于告诉编译器gcc该函数不会返回.这样可让gcc产生更好的代码,更重要的是使用这个关键字
// 可以避免产生某些(未初始化变量的)假警告信息.
...
// 刷新页变换高速缓冲宏函数.
// 为了提高地址转换的效率,CPU将最近使用的页表数据存放在芯片中高速缓冲中.在修改过页表信息之后,就需要刷新该缓冲区.
// 这里使用重新加载页目录重新加载页目录基址寄存器cr3的方法来进行刷新.下面eax = 0,是页目录的基址.
