...
// 历史上最早在PDP-11计算机上,魔数(幻数)是八进制数0407(0x107).它位于执行程序头结构开始处.原本是PDP-11的一条跳
// 转指令,表示跳转到随后7个字后的代码开始处.这样加载程序(loader)就可以在把执行文件放入内存后直接跳转到指令开
// 始处运行.现在已没有程序使用这种方法,但这个八进制数却作为识别文件类型的标志(魔数)保留了下来.OMAGIC可以认为
// 是Old Magic的意思
#define OMAGIC 0407
/* Code indicating pure executable.  */
/* 指明为纯可执行文件的代号 */
/* New Magic,1975年以后开始使用.涉及虚存机制 */
#define NMAGIC 0410
/* Code indicating demand-paged executable.  */
/* 指明为需求分页处理的可执行文件 */ // 其头结构占用文件开始处1K空间
#define ZMAGIC 0413
#endif /* not OMAGIC */
// 另外还有一个OMAGIC,是为节约磁盘容量,把盘上执行文件的头结构与代码紧凑存放.下面宏用于判断魔数字段的正确性.如
// 果魔数不能被识别,则返回真.
...
// 代码部分起始偏移值.
// 如果文件是ZMAGIC类型的,即是执行文件,那么代码部分是从执行文件的1024字节偏移处开始.否则执行代码部分紧随执行头
// 结构末端(32字节)开始,即文件是模块文件(OMAGIC类型).
#ifndef N_TXTOFF
#define N_TXTOFF(x) \
