...
// 下面是sigaction结构sa_flags标志字段可取的符号常数值。
...
// 以下两个常数符号都表示指向无返回值函数指针，且都有一个int整型参数。这两个指针值是逻辑上讲实际上不可能出现的
// 函数地址值。可作为下面signal函数的第二个参数。用于告知内核，让内核处理信号或忽略对信号的处理。
...
struct sigaction {
	void (*sa_handler)(int);	// 对应某信号指定要采取的行动。可以用上面的SIG_DEL，或SIG_IGN来忽略该信号，
								// 也可以是指向处理该信号函数的一个指针
...
// 下面signal函数用于是为信号_sig安装一新的信号处理程序（信号句柄），与sigaction()类似。该函数含有两个参数：指定需
// 要捕获的信号_sig；具有一个参数且无返回值的函数指针_func。该函数返回值也是具有一个int参数（最后一个(int)）且无返
// 回值的函数指针，它是处理该信号的原处理句柄。
