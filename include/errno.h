...
// 系统调用以及很多库函数返回一个特殊的值以表示操作失败或出错.这个值通常选择-1或者其他一些特定的值来表示.但是这个
// 返回值仅说明错误发生了.如果需要知道出错的类型,就需要查看表示系统出错号的变量errno.该变量即在errno.h文件中声明.
// 在程序开始执行时该变量值被初始化为0.
extern int errno;

// 在出错时，系统调用会把出错号放在变量erron中（负值），然后返回-1.因此程序若需要知道具体错误号，就需要查看errno
// 的值。
