 ...
 */
/*
 * 注意!!!我们有时短暂地禁止和允许中断(当输出一个字(word)到视频IO)，但即使对于键盘中断这也是可以工作的。因为我们使
 * 用了陷阱门。所以我们知道在处理一个键盘过程期间中断是被禁止的.希望一切均正常.
 */
...
// 虚拟控制台结构.其中包含一个虚拟控制台的当前所有信息.其中vc_origin和vc_scr_end是当前正在处理的虚拟控制台执行快速
// 滚屏操作时使用的起始行和末行对应的显示内存位置.vc_video_mem_start和vc_video_meme_end是当前虚拟控制台使用的显示
// 内存区域部分.
static struct {
...
// 为人便于引用,以下定义当前正在处理控制台信息的符号.含义同上.其中currcons是使用vc_cons[]结构的函数参数中的当前
// 虚拟终端号.
...
// 主机通过发送不带参数或参数是0的设置属性(DA)控制序列('ESC [c'或'ESC [0c']要求终端应答一个设备属性控制序列(ESC Z的
// 作用与此相同)终端则发送以下序列来响应主机.该序列(即'ESC [?1;2c')表示终端是具有高级视频功能的VT100兼容终端.
#define RESPONSE "\033[?1;2c"

// 使用的字符集.其中上半部分时普通7位ASCII码,即US字符集.下半部分对应VT100终端设备中的线条字符,即显示图表线条的字符集.
...
// 更新当前光标位置变量x,y,并修正光标在显示内存中的对应位置pos.该函数会首先检查参数的有效性.如果给定的光标列号超出显
// 示器最大列数,或者光标行号不低于显示的最大行数,则退出.否则就更新当前光标变量和新光标位置对应在显示内存中位置pos.
...
{
	// 首先判断显示卡类型.对于EGA/VGA,我们可以指定屏内范围(区域)进行滚屏操作,而MDA单色显示卡只能进行整屏滚屏操作.
	// 因此只有EGA/VGA卡才需要设置滚屏起始行显示内存地址(起始行是origin对应的行).即显示类型如果不是EGA/VGA彩色模
	// 式,也不是EGA/VGA单色模式,那么就直接返回.另外,我们只对前台控制台进行操作,因此当前控制台currocons必须是前台
	// 控制台时,我们才需要设置其滚屏起始行对应的内存起点位置.
	if (video_type != VIDEO_TYPE_EGAC && video_type != VIDEO_TYPE_EGAM)
		return;
	if (currcons != fg_console)
		return;
	// 然后向显示寄存器选择端口video_port_reg输出12,即选择显示控制数据寄存器r12,接着写入滚屏起始地址高字节.其中向
	// 右移动9位,实际上表示向右移动8位再除以2(上1个字符用2字节表示).再选择显示控制数据寄存器r13,然后写入滚屏起始
	// 地址低字节.向右移动1位表示除以2,同样代表屏幕上1个字符用2字节表示.输出值相对于默认显示内存起始位置
	// video_mem_base进行操作.例如对于EGA/VGA彩色模式,viedo_mem_base = 物理内存地址0xb8000.
	cli();
...
{
	// 滚屏区域必须至少有2行.如果滚屏区域顶行号大于等于区域底行号,则不满足进行滚行操作的条件.另外,对于EGA/VGA卡,
	// 我们可以指定屏内行范围(区域)进行滚屏操作,而MDA单色显示卡只能进行整屏操作.该函数对EGA和MDA显示类型进行分别
	// 处理.如果显示类型是EGA,则还分为整屏窗口移动和区域内窗口移动这里首先处理显示卡是EGA/VGA显示类型的情况.
	if (bottom <= top)
		return;
	if (video_type == VIDEO_TYPE_EGAC || video_type == VIDEO_TYPE_EGAM)
	{
...
void csi_m(int currcons)
{
	...
	for (i = 0; i <= npar; i++)
		switch (par[i]) {
			...
			// 当Ps(par[i])为其他值时,则是设置指定的前景色或背景色.如果Ps = 30..37,则是设置前景色;如果Ps=40..47,
			// 则是设置背景色.
			default:
			  if (!can_do_colour)
			    break;
